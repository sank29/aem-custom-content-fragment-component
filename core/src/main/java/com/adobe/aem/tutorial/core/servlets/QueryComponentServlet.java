...
public class QueryComponentServlet extends SlingAllMethodsServlet {
	...
	static String getPath(String path) {
		...
		if (path == null || !path.contains("/jcr:content")) {
            return path; // return as-is if no jcr:content
        }
	...
	protected void doPost(final SlingHttpServletRequest req, final SlingHttpServletResponse resp)
		...
		predicate.put("property.value","practice/components/servletcomponent");
		...
		for(Hit eachHit: hits) {
			...
			try {
					jsonObject.addProperty("path", getPath(eachHit.getPath()));
